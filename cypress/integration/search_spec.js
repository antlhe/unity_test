        ...
        context('Middle Search Bar', () => {
                ...
                context('Category', () => {
                    ...
                    it('should have all the categories stated in the requirements (wireframe sketch)', function () {
                        cy.get('#cid').children('option').then(options => {
                            ...
                            expect(actual.sort().toString()).to.deep.eq(REQUIRED_CATEGORIES.sort().toString())
                        })
                    })
                    ...
                    it('should have categories sorted alphabetically', function () {
                        cy.get('#cid').children('option').then(options => {
                            ...
                            //toString() for better error reporting
                            expect(actual.toString()).to.deep.eq(actualSorted.toString())
                        })
                    })
                ...
                context('Manufacturer', () => {
                    ...
                    it('should have all the manufacturers stated in the requirements (wireframe sketch)', function () {
                        cy.get('#mid').children('option').then(options => {
                            const actual = [...options].map(o => o.text)
                            //toString() for better error reporting
                            expect(actual.sort().toString()).to.deep.eq(REQUIRED_MANUFACTURERS.sort().toString())
                        })
                    })
            ...
            context('Product Selectors', () => {
                ...
                context('Products Sort By', () => {
                    ...
                    it('should sort the products alphabetically when selecting `Name: A to Z` Products Sort By option', function () {
                        cy.get('#products-orderby').select('Name: A to Z');
                        cy.get('div.item-grid').children().children().children('div.details').children('h2.product-title').children().then(productTitles => {
                            const productTitlesArray = [...productTitles].map(o => o.text)
                            const productTitlesArraySorted = [...productTitlesArray].sort()
                            //toString() for better error reporting
                            expect(productTitlesArray.toString()).to.deep.eq(productTitlesArraySorted.toString())
                        })
                    })

                    it('should sort the products in reverse alphabetical order when selecting `Name: A to Z` Products Sort By option', function () {
                    ...
                    it('should sort the products appropriately when selecting `Price: Low to High` Products Sort By option', function () {
                        cy.get('#products-orderby').select('Price: Low to High');
                        cy.get('div.item-grid').children().children().children('div.details').children('div.add-info').children('div.prices').children('span.price.actual-price').then(productPrices => {
                            ...
                            const productPricesArraySorted = [...productPricesArray].sort(function(a,b) { return a - b;})
                            //toString() for better error reporting
                            expect(productPricesArray.toString()).to.deep.eq(productPricesArraySorted.toString())
                            
                        })
                    })

                    it.only('should sort the products appropriately when selecting `Price: High to Low` Products Sort By option', function () {
                        ...
                        cy.get('div.item-grid').children().children().children('div.details').children('div.add-info').children('div.prices').children('span.price.actual-price').then(productPrices => {
                            ...
                            const productPricesArraySorted = [...productPricesArray].sort(function(a,b) { return a - b;}).reverse()
                            //toString() for better error reporting
                            expect(productPricesArray.toString()).to.deep.eq(productPricesArraySorted.toString())
                ...
                context('Products Page Size', () => {
                    ...
                    context('Pagination', () => {
                        ...
                        it('should navigate to the correct page when selecting the pagination `first page` (i.e. `<<`)', function () {
                            cy.get('li.last-page').click();
